...
int main(){
	...
	// variables for reading in data from serial
	char rx_buffer[256], tx_buffer[40];
	...
	while(true){
		...
		tx_length = write(uart0_filestream, &tx_buffer[0], strlen(tx_buffer));
		
		if(tx_length != (int)strlen(tx_buffer)){
			cout << "something bad happened to sent data" << endl;
		}
