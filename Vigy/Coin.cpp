...
int main(){
	...
	while(true){
		...
		for(auto& cnt : contours){
			...
			if(contourArea(cnt) < 3600 || contourArea(cnt) > 8600){
			...
			if(br.height/(float)br.width < 0.90 || br.height/(float)br.width > 1.05 ){
				drawContours(imgafterthreshcont,vector<vector<Point>>(1,cnt),-1,Scalar(0,0,225), 2);
				continue;
				
			}
			//Now we have gotten rid of any random objects
			if(contourArea(cnt) >= 7200){
				total = total + 25;
			}
			else if(contourArea(cnt) >= 5500){
				total = total + 5;
			}
			else if(contourArea(cnt) >= 4300 ){
				total = total + 1;
			}
			else if(contourArea(cnt) >= 3600 ){
				total = total + 10;
			}
			printf("TOTAL: %d \n", total);
		}
