...
int green_square(Mat img){
		...
		Mat imgwline;
		Mat imgwgreen;
		Scalar LowerGreen = Scalar (78, 139, 34);
		Scalar HighGreen = Scalar (83, 255, 204);
		int leftgreen = 0;
		int rightgreen = 0;
		int top = 0;
		int bottom = 0;
	...
	if(contours.size() != 0){
			...
			for(auto& cnt : contours){
				...
				if(contourArea(cnt) < 5000|| contourArea(cnt) > 10500){
					continue;
				}
				else{
					//If detect green
	...
	if((int)imgwline.at<uchar>(Point(center.x,center.y)) == 255){//You are on good path or need a sharp turn
		for(int i = 130; i < 159; i++){//checks wheter you need to take a sharp turn right
			if((int)imgwline.at<uchar>(Point((center.x + i),center.y)) == 255){//Line on the right
				if(imgwline.at<uchar>(Point(center.x,newheight/4)) != 255){//REAL TURN
